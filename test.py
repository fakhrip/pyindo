...
if __name__ == "__main__":
    ...
    for iteration, program, output in zip(range(len(incorrect_programs)), incorrect_programs, incorrect_programs_out):
    ...
    for iteration, program, output in zip(range(len(correct_programs)), correct_programs, correct_programs_out):
        program_out = popen(f"python3 main.py {program}").read().lstrip("\n")
        expected_out = open(output, "r").read()
